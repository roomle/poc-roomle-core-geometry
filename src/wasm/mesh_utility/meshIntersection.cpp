...
TrianglePlaneIntersection roomle::mesh::intersectTriangleAndPlane(
        const float p0[],
        const float p1[],
        const float p2[],
...
TriangleTriangleIntersection roomle::mesh::intersectTriangles(
        const float t0p0[],
        const float t0p1[],
        const float t0p2[],
        const float t1p0[],
...
void MeshIntersection::intersectTrianglesOfMeshes(
    ...
    for (uint32_t iiT0 = 0; iiT0 < intersectingTrianglesOfMesh0.size(); ++iiT0) {
        ...
        for (uint32_t iiT1 = 0; iiT1 < intersectingTrianglesOfMesh1.size() && intersectingTrianglesOfMesh1[iiT1] < noOfOriginalTrianglesMesh1; ++iiT1) {
            ...
            auto intersectionDirection = normalize3(cross3(triangle1.faceNormal, triangle0.faceNormal));
            ...
            if (length3(directionVector) < epsilon)
                continue;
...
std::tuple<Vector3, Vector2> MeshIntersection::calculateNormalAndUVForPointInTriangle(
    ...
    if (!mesh.normals.empty()) {
        ...
        nv = normalize3(interpolateBarycentric3(barycentricCoordinates, n0, n1, n2));
    }
    Vector2 uv;
    if (!mesh.uvs.empty()) {
