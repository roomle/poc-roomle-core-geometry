import { MeshSpecification } from './geometry_buffer';
import { 
...
export const barycentricCoordinate = (ab: Vector3, ac: Vector3, ax: Vector3): Vector3 => {
    const d00 = ab.dot(ab);
    const d01 = ab.dot(ac);
    const d11 = ac.dot(ac);
    const d20 = ax.dot(ab);
...
export const interpolateBarycentric = (b: Vector3, t: Vector3[]): Vector3 => {
    const v = new Vector3(0, 0, 0);
    v.add(t[0].clone().multiplyScalar(b.x)).add(t[1].clone().multiplyScalar(b.y)).add(t[2].clone().multiplyScalar(b.z));
    return v;
}

export const planeFromTriangle = (p0: Vector3, p1: Vector3, p2: Vector3): Plane => {
    return { 
...
export const intersectRayPlaneByThreePoints = (ray: Ray, p0: Vector3, p1: Vector3, p2: Vector3): Intersection | null => {
    return intersectRayPlane(ray, planeFromTriangle(p0, p1, p2));
}

export const transformRay = (ray: Ray, m: Matrix4) => {
    return {
        origin: ray.origin.applyMatrix4(m), 
        direction: ray.direction.applyMatrix3(new Matrix3().setFromMatrix4(m))
    }
}

// https://stackoverflow.com/questions/46749675/opengl-mouse-coordinates-to-space-coordinates/46752492#46752492
export const createViewSpaceRay = (camera: Camera, ndc2dCoordinate: Vector2): Ray => {
    if (camera instanceof OrthographicCamera) {
        const orthographicCamera = camera as OrthographicCamera;
        const pointOnNearPlane = new Vector3(ndc2dCoordinate.x, ndc2dCoordinate.y, -1).applyMatrix4(orthographicCamera.projectionMatrixInverse);
        const pointOnFarPlane = new Vector3(ndc2dCoordinate.x, ndc2dCoordinate.y, 1).applyMatrix4(orthographicCamera.projectionMatrixInverse);
        const direction = pointOnFarPlane.sub(pointOnNearPlane);
        return { origin: pointOnNearPlane, direction };
    } 
    const perspectiveCamera = camera as PerspectiveCamera;
    const origin = new Vector3(0, 0, 0);
    const pointOnRay = new Vector3(ndc2dCoordinate.x, ndc2dCoordinate.y, 0).applyMatrix4(perspectiveCamera.projectionMatrixInverse);
    const direction = pointOnRay.sub(origin);
    return { origin, direction };
}

export const createWorldSpaceRay = (camera: Camera, ndc2dCoordinate: Vector2): Ray => {
...
export const intersectRayBox = (ray: Ray, box: Box3): boolean => { 
...
const pointInOrOn = (px: Vector3, p0: Vector3, a: Vector3, b: Vector3): boolean => {
    const cp1 = b.clone().sub(a).cross(px.clone().sub(a));
    const cp2 = b.clone().sub(a).cross(p0.clone().sub(a));
    return cp1.dot(cp2) >= 0;
}

const pointInTriangle = (px: Vector3, p0: Vector3, p1: Vector3, p2: Vector3): boolean => {
...
export const intersectTriangle = (ray: Ray, tPts: Vector3[], maxDistance?: number): Intersection | null => {
...
export const findTriangleInMesh = (mesh: Mesh, camera: Camera, ndc2dCoordinate: Vector2): Vector3[] | null => {
    ...
    const vertices = (mesh.geometry.attributes.position as BufferAttribute).array;
    const indices = mesh.geometry.index?.array;
    
    const vertex = (i: number) => new Vector3(vertices[i*3], vertices[i*3+1], vertices[i*3+2]);
    let minimumIntersection: Intersection | undefined;
    let triangleVertices: Vector3[] = [new Vector3(0, 0, 0)];
...
export const calculateIntersectionBox = (mesh1: Mesh, mesh2: Mesh, relativeToMesh1?: boolean): Box3 => {
...
export const trianglesInBox = (mesh: Mesh, searchBox: Box3): number[] => {
    if (!mesh.geometry.boundingBox) {
        mesh.geometry.computeBoundingBox();
    }
    if (!mesh.geometry.boundingBox) {
    ...
    for (let ti = 0; ti < indices.length; ti += 3) {
        ...
        const triangleBox = new Box3().expandByPoint(v0).expandByPoint(v1).expandByPoint(v2);
...
export const intersectTrianglePlane = (t: Vector3[], plane: Plane): TrianglePlaneIntersection | null => {
...
export const intersectTriangles = (t1: Vector3[], t2: Vector3[]): TriangleTriangleIntersection | null => {
...
export class Triangle {
    ...
    private _vertices: Vector3[] | undefined;
    private _box: Box3 | undefined; 
    private _normalVector: Vector3 | undefined; 
    ...
    public applyMatrix(transformation: Matrix4) {
    ...
    public barycentricCoordinate(p: Vector3): Vector3 {
    ...
    private createVertexByIndex(i: number) {
        return new Vector3(this.vertexArray[i*3], this.vertexArray[i*3+1], this.vertexArray[i*3+2]);
    }  
}
...
export class UniqueIndices {
    ...
    public getVertexIndex(v: Vector3, epsilon: number = 0.001): number {
...
export const intersectMesh = (mesh1: Mesh, mesh2: Mesh, epsilon = 0.0001): MeshSpecification | null => {
    //const wasmResult = intersectMeshWASM(mesh1, mesh2, MESH_INTERSECT, epsilon);
    //if (wasmResult !== null) {
    //    return wasmResult;
    //}

    const vertices1 = (mesh1.geometry.attributes.position as BufferAttribute).array;
    const indices1Buffer = (mesh1.geometry.index as BufferAttribute)?.array;
    const vertex2Buffer = (mesh2.geometry.attributes.position as BufferAttribute).array;
    const indices2 = mesh2.geometry.index?.array;
    if (!indices1Buffer || !indices2) {
        return null;
    }
    const normals1 = (mesh1.geometry.attributes.normal as BufferAttribute)?.array;
    ...
    for (let i = 0; i < vertex2Buffer.length; i += 3) {
        let v = new Vector3(vertex2Buffer[i], vertex2Buffer[i+1], vertex2Buffer[i+2]);
    ...
    const pushNormals = (normals: ArrayLike<number> | undefined, indices: number[], barycentric: Vector3) => {
        if (normals) {
            const nv = (i: number) => new Vector3(normals[i*3], normals[i*3+1], normals[i*3+2]); 
            const n = interpolateBarycentric(barycentric, indices.map(i => nv(i)));
            newNormals.push(n.x, n.y, n.z);
        }
    }
    const pushNewVertex = (newVertex: Vector3, triangle: Triangle) => {
    ...
    const pushTriangle = (intersectionDirection: Vector3, i0: number, i1: number, i2:number) => {
        const uniqueI = [i0, i1, i2].map(i => mesh1UniqueIndices.get(i)); 
        if (uniqueI[0] == uniqueI[1] || uniqueI[0] == uniqueI[2] || uniqueI[1] == uniqueI[2]) {
            return;
        }
        const v = [i0, i1, i2].map(i => new Vector3(newVertices[i*3], newVertices[i*3+1], newVertices[i*3+2]));
...
export const intersectMeshWASM = (mesh0: Mesh, mesh1: Mesh, operator: number, epsilon = 0.0001): MeshSpecification | null => {
    ...
    const meshData1 = {
        vertices: (mesh1.geometry.attributes.position as BufferAttribute).array,
        normals: (mesh1.geometry.attributes.normal as BufferAttribute)?.array,
        uvs: (mesh1.geometry.attributes.uv as BufferAttribute)?.array,
    ...
    if (relativeTransform) {
        ...
        for (let i = 0; i < meshData1.vertices.length; i += 3) {
            let v = new Vector3(meshData1.vertices[i], meshData1.vertices[i+1], meshData1.vertices[i+2]);
            v = v.applyMatrix4(relativeTransform);
            transformedVertices.push(v.x, v.y, v.z);
        }
        const normalMatrix = new Matrix3().getNormalMatrix(relativeTransform);
        const transformedNormals: number[] = [];
        for (let i = 0; i < meshData1.normals.length; i += 3) {
            let n = new Vector3(meshData1.normals[i], meshData1.normals[i+1], meshData1.normals[i+2]);
